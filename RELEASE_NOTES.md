...
### 🆕 Поля релизов в задачах

`create_issue` и `update_issue` получили два новых параметра для привязки задач к релизам (множественные ассоциации, источник `emodel/ecos-release-type`):
...
Принимают список рефов релизов (UUID или полный `emodel/ecos-release-type@...`). Рефы можно получить через `query_releases`.
