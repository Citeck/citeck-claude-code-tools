...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(description="Query Citeck Project Tracker tags")
...
def format_tags(records):
    """Format tag records as a readable table."""
    if not records:
...
def main(argv=None):
    ...
    query = {"t": "eq", "att": "_type", "val": "emodel/type@ept-tags"}
    ...
    print(f"Found {len(records)} tag{'s' if len(records) != 1 else ''} in {project}")
    print()
    print(format_tags(records))
